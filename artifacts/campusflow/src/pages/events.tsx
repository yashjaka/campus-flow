...
const typeColor: Record<string, string> = {
  ...
  "Cultural Event": "bg-pink-500/20 text-pink-400",
  "Sports Event": "bg-orange-500/20 text-orange-400",
    ...
    setTimeout(() => {
      if (editing) {
        ...
        notificationStore.add({
          ...
          message: `${ev.type} on ${ev.date} at ${ev.venue}. Register now!`,
          link: "/events",
        });
      }
      refresh();
                  ...
                  <Card className="glass-card flex flex-col h-full hover:border-primary/20 transition-all">
                    <CardContent className="p-5 flex-1 flex flex-col">
                      <div className="flex items-start justify-between mb-3">
                        <Badge
                          className={`text-xs ${typeColor[ev.type] ?? "bg-muted/20 text-muted-foreground"}`}
                        >
                      ...
                      <div className="space-y-1 text-xs text-muted-foreground mb-4">
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          {ev.attendees.length} registered
                        </div>
                        <div>
                          By {ev.createdBy} · {ev.createdByRole}
                        </div>
                      </div>
                      {user?.role === "student" && (
