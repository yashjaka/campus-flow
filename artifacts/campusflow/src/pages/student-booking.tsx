              ...
              {dayBookings.length > 0 && (
                <div className="absolute bottom-1 left-1/2 -translate-x-1/2 flex gap-0.5">
                  {dayBookings.slice(0, 3).map((_, j) => (
                    <span
                      key={j}
                      className={`w-1 h-1 rounded-full ${RESOURCE_COLORS[j % RESOURCE_COLORS.length]?.replace("/20", "/80") ?? "bg-primary"}`}
                    />
                  ))}
                </div>
              )}
                ...
                {selectedDayBookings.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {selectedDayBookings.map((b) => (
                      <div
                        key={b.id}
                        className="flex items-center justify-between p-2 bg-muted/10 rounded-lg text-sm"
                      >
                        <div>
                          <p className="font-medium">{b.resource}</p>
                          <p className="text-xs text-muted-foreground">
                            {b.timeSlot} · {b.studentName}
                          </p>
                        </div>
                        <Badge className="text-xs border bg-green-500/20 text-green-400 border-green-500/30">
                          Approved
                        </Badge>
                      </div>
                    ))}
...
export default function StudentBooking() {
  ...
  const [myBookings, setMyBookings] = useState<Booking[]>(() =>
    user ? bookingStore.getByStudent(user.id) : [],
  );
  ...
  const refresh = useCallback(() => {
    if (user) setMyBookings(bookingStore.getByStudent(user.id));
  }, [user]);

  const checkConflict = (resource: string, date: string, timeSlot: string) => {
    if (resource && date && timeSlot)
      setConflict(bookingStore.isSlotTaken(resource, date, timeSlot));
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      addNotification({
        ...
        message: `Your request for ${form.resource} on ${form.date} (${form.timeSlot}) is pending approval.`,
      ...
      setTimeout(() => {
        setSubmitted(false);
        setForm({ resource: "", date: "", timeSlot: "", purpose: "" });
      }, 2500);
    }, 500);
  };
      ...
      <div className="space-y-5">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Book campus facilities for academic and co-curricular activities.
          </p>
        </div>
                        ...
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            <div className="flex items-center gap-2 mb-1">
                              <Badge
                                className={`text-xs border ${statusColor[b.status]}`}
                              >
                                {b.status}
                              </Badge>
                            </div>
                ...
                <AnimatePresence mode="wait">
                  {submitted ? (
                    ...
                    >
                      ...
                      <p className="text-sm text-muted-foreground">
                        Awaiting approval from faculty or admin.
                      </p>
                    </motion.div>
                  ) : (
                    <motion.form
                    ...
                    >
                      <div className="space-y-2">
                        ...
                        <Select
                          value={form.resource}
                          onValueChange={(v) => {
                            setForm((f) => ({ ...f, resource: v }));
                            checkConflict(v, form.date, form.timeSlot);
                          }}
                        >
                          <SelectTrigger>
                      ...
                      <div className="space-y-2">
                        ...
                        <Select
                          ...
                          onValueChange={(v) => {
                            ...
                            checkConflict(form.resource, form.date, v);
                          }}
                        >
                          <SelectTrigger>
