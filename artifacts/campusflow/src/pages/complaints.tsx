...
function ImageUpload({
  ...
  return (
    <div>
      ...
      <div
        className="border-2 border-dashed border-border rounded-lg p-3 cursor-pointer hover:border-primary/50 transition-colors text-center"
...
export default function Complaints() {
  ...
  const refresh = useCallback(() => {
    ...
    if (isMaintenance)
      issues = issues.filter((i) => i.assignedTo?.name === user?.name);
    setAllIssues(issues);
  }, [isMaintenance, user]);
  ...
  const openDetail = (issue: Issue) => {
    ...
    setAssignedTo(
      issue.assignedTo
        ? `${issue.assignedTo.name}||${issue.assignedTo.role}`
        : "",
    );
  ...
  const handleSave = () => {
    ...
    const assignedUser =
      assignedTo && assignedTo !== "__unassigned__"
        ? staffList.find((s) => `${s.name}||${s.role}` === assignedTo)
        : undefined;
    ...
    if (assignedUser && selectedIssue.status === "Submitted")
  ...
  const subtitle = isMaintenance
    ...
    : "View, assign, and resolve campus complaints";
    ...
    <DashboardLayout>
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
          {STATUS_ORDER.map((s) => (
            <button
              ...
              className={`p-2 rounded-lg text-center border transition-all ${statusFilter === s ? "border-primary bg-primary/10" : "border-border bg-muted/10 hover:border-primary/30"}`}
            >
        ...
        <Sheet open={detailOpen} onOpenChange={setDetailOpen}>
          <SheetContent
            side="right"
            className="w-full sm:max-w-xl overflow-y-auto glass-card border-l border-border/50 p-0"
          >
            {selectedIssue && (
              <>
                <SheetHeader className="p-6 pb-4 border-b border-border/30">
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    <Badge
                      className={`border ${statusColor[selectedIssue.status]}`}
                    >
                ...
                <div className="p-6 space-y-5">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    {selectedIssue.description && (
                      <p className="text-muted-foreground bg-muted/10 p-3 rounded-lg text-xs mt-2">
                        {selectedIssue.description}
                      </p>
                    )}
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Textarea
                        placeholder="Describe the resolution or updates..."
                        rows={3}
