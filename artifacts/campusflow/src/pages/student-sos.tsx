...
const SOS_TYPES: Array<{ type: SosType; icon: React.ComponentType<{ className?: string }>; color: string; bg: string }> = [
  { type: 'Medical Emergency', icon: Heart, color: 'text-red-400', bg: 'bg-red-500/20 border-red-500/40 hover:bg-red-500/30' },
  { type: 'Fire', icon: Flame, color: 'text-orange-400', bg: 'bg-orange-500/20 border-orange-500/40 hover:bg-orange-500/30' },
  { type: 'Security Threat', icon: ShieldAlert, color: 'text-yellow-400', bg: 'bg-yellow-500/20 border-yellow-500/40 hover:bg-yellow-500/30' },
  { type: 'Electrical Hazard', icon: Zap, color: 'text-blue-400', bg: 'bg-blue-500/20 border-blue-500/40 hover:bg-blue-500/30' },
  { type: 'Other', icon: AlertTriangle, color: 'text-purple-400', bg: 'bg-purple-500/20 border-purple-500/40 hover:bg-purple-500/30' },
];
      ...
      <div className="space-y-6 max-w-2xl mx-auto">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-1">Report a campus emergency immediately.</p>
          </div>
              ...
              <Card className="glass-card border-red-500/20 overflow-hidden">
                <CardContent className="p-8 flex flex-col items-center text-center">
                  ...
                  >
                    ...
                    <span className="text-2xl font-black text-red-400 tracking-widest">SOS</span>
                  </motion.button>
                  <p className="text-muted-foreground mt-6 max-w-xs text-sm">
                    Tap the button above to report an emergency. Campus security will be notified immediately.
                  </p>
                </CardContent>
              </Card>
            ...
            <motion.div key="confirmed" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}>
              <Card className="glass-card border-green-500/30">
                <CardContent className="p-8 flex flex-col items-center text-center gap-4">
                  ...
                  <div>
                    <h2 className="text-2xl font-bold text-green-400">Alert Sent!</h2>
                    <p className="text-muted-foreground mt-2">
                      Your <strong>{lastAlert.type}</strong> alert has been dispatched. Campus security has been notified.
                    </p>
                  </div>
                  <div className="bg-muted/20 rounded-lg p-3 w-full text-sm text-left space-y-1">
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Alert ID</span>
                      <span className="font-mono text-xs">{lastAlert.id.split('-')[0]}</span>
                    </div>
                    <div className="flex justify-between">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span>{new Date(lastAlert.createdAt).toLocaleTimeString()}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Status</span>
                      <Badge className="text-xs border bg-red-500/20 text-red-400 border-red-500/30">Active</Badge>
                    </div>
                  </div>
        ...
        {showHistory && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
            <Card className="glass-card">
              ...
              <CardContent>
                {history.length === 0 ? (
                  <p className="text-sm text-muted-foreground text-center py-4">No emergency alerts sent.</p>
                ) : (
                  <div className="space-y-3">
                    {history.map((alert) => (
                      <div key={alert.id} className="flex items-start justify-between gap-3 p-3 bg-muted/10 rounded-lg">
                        <div>
                          <div className="flex items-center gap-2 mb-1">
                            <Badge className={`text-xs border ${statusColor[alert.status]}`}>{alert.status}</Badge>
