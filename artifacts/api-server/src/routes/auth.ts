...
import { StudentEntryBody, StaffLoginBody, AdminLoginBody } from "@workspace/api-zod";
...
router.post("/auth/student-entry", async (req, res) => {
  try {
    ...
    const token = signToken({ userId: student.id, role: "student", name: student.name });
...
router.post("/auth/staff-login", async (req, res) => {
  try {
    ...
    const valid = staff.passwordHash && await bcrypt.compare(parsed.data.password, staff.passwordHash);
    if (!valid) {
      res.status(401).json({ error: "Invalid credentials" });
      return;
    }

    const token = signToken({ userId: staff.id, role: staff.role, name: staff.name });
...
router.post("/auth/admin-login", async (req, res) => {
  try {
    ...
    const valid = admin.passwordHash && await bcrypt.compare(parsed.data.password, admin.passwordHash);
    if (!valid) {
      res.status(401).json({ error: "Invalid credentials" });
      return;
    }
