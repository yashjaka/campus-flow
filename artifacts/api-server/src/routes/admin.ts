...
router.get("/admin/stats", requireAuth, requireAdmin, (_req, res) => {
  ...
  const totalMaintenance = allUsers.filter((u) => u.role === "maintenance").length;
  const totalAdmins = allUsers.filter((u) => u.role === "admin").length;
  const recentRegistrations = allUsers.filter(
    (u) => u.role === "student" && u.createdAt >= sevenDaysAgo,
  ).length;

  res.json({ totalStudents, totalFaculty, totalMaintenance, totalAdmins, recentRegistrations });
});
